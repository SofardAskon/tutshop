@foreach ($categories as $category)
    <li class="category-header__item">
        <a href="{{ route('shop.category', ['id' => $category->id]) }}" class="category-header__link">{{ $category->title }}</a>
        <ul class="category-header__sub-list">
            @isset($category->children)
                @foreach ($category->children as $categoryChildren)
                    <li class="category-header__sub-item">
                        <a href="{{ route('shop.category', ['id' => $categoryChildren->id]) }}" class="category-header__sub-link">
