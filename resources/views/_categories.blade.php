@foreach ($categories as $category)
    <li class="content-catalog__item">
        <a href="{{ route('shop.category', ['id' => $category->id]) }}" class="content-catalog__link">
        ...
        <div class="content-catalog__wrap wrap-content-catalog" data-simplebar>
            <a href="{{ route('shop.category', ['id' => $category->id]) }}" class="wrap-content-catalog__title">{{ $category->title }}
                ...
                <ul class="content-catalog__sub-list">
                    @isset($category->children)
                        @foreach ($category->children as $categoryChildren)
                            <a href="{{ route('shop.category', ['id' => $categoryChildren->id]) }}" class="content-catalog__sub-title">{{ $categoryChildren->title }}</a>
                            @foreach ($categoryChildren->children as $categoryChildrenThree)
                                <li class="content-catalog__sub-item"><a href="{{ route('shop.category', ['id' => $categoryChildrenThree->id]) }}" class="content-catalog__sub-link">{{ $categoryChildrenThree->title }}</a></li>
                            @endforeach
                        @endforeach
                    @endisset
                </ul>
